...
def step_back(multigraph: nx.MultiDiGraph, u: int, v: int, key: int, w_edges: list) -> None:
...
def backtracking_1(multigraph: nx.MultiDiGraph, x_edges, y_edges, vertex, timeout: tuple = None,
    ...
    for u, v, key, attrs in filter(utils.is_non_fixed_edge, multigraph.edges(vertex, data=True, keys=True)):
        ...
        if (
                'included_in_z' in multigraph.nodes[v]
        ...
        added_to_w = []
        for source in (
                multigraph.edges(vertex, data=True, keys=True), multigraph.in_edges(vertex, data=True, keys=True)):
        ...
        if multigraph.graph['length_z'] == len(x_edges) and multigraph.graph['length_w'] == len(x_edges):
            z_edges = set(item[:2] for item in nx.get_edge_attributes(multigraph, 'fixed_z'))
...
def simple_path(graph_x: list, graph_y: list, timeout: tuple = None, global_timeout: tuple = None) -> bool:
    """
    ...
    start_node = 1
    for in_edge, out_edge in itertools.product(multigraph.in_edges(start_node), multigraph.edges(start_node)):
        if in_edge == out_edge:
            continue
        ...
        if backtracking_1(multigraph, x_edges, y_edges, out_edge[1], timeout=timeout, global_timeout=global_timeout):
            return True
...
def backtracking_2(multigraph, x_edges, y_edges, edge, timeout: tuple = None, global_timeout: tuple = None) -> bool:
    ...
    if len(z) == len(x_edges) and len(w) == len(x_edges):
        z_edges = set(item[:2] for item in z)
        w_edges = set(item[:2] for item in w)
