...
@pytest.mark.parametrize(
    'first,second,expected', [(1, 2, True), (8, 28, True), (7, 3, False)]
)
...
def test_edges_equal(first, second, third, expected):
    ...
    graph_1.edges.append(
        undirected.Edge({undirected.Vertex(first), undirected.Vertex(second)})
    )
    graph_2.edges.append(
        undirected.Edge({undirected.Vertex(first), undirected.Vertex(third)})
    )
...
def test_is_hamiltonian_cycle(expected):
    ...
    if expected:
        graph.edges.append(
            undirected.Edge({undirected.Vertex(3), undirected.Vertex(1)})
        )
    else:
        graph.edges.append(
            undirected.Edge({undirected.Vertex(3), undirected.Vertex(5)})
        )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ([2, 1, 6, 5, 3, 4, 7], [1, 2, 3, 7, 4, 5, 6], False),
        ([7, 5, 2, 4, 1, 3, 6], [7, 1, 5, 3, 6, 4, 2], True),
        ([6, 3, 2, 4, 5, 1, 7], [7, 4, 5, 2, 1, 6, 3], False),
        ([2, 4, 1, 7, 6, 5, 3], [7, 1, 6, 5, 2, 3, 4], True),
        ([5, 3, 2, 7, 6, 4, 1], [7, 1, 3, 6, 4, 5, 2], True),
        ([2, 6, 1, 5, 7, 4, 3], [3, 7, 4, 5, 6, 2, 1], True),
        ([3, 7, 6, 5, 4, 1, 2], [5, 6, 2, 1, 3, 7, 4], False),
        ([5, 4, 3, 6, 2, 7, 1], [4, 6, 5, 7, 2, 1, 3], False),
        ([3, 5, 1, 2, 4, 7, 6], [5, 7, 3, 2, 6, 1, 4], True),
        ([4, 3, 6, 7, 2, 5, 1], [3, 5, 1, 6, 2, 7, 4], False),
    ),
)
...
def test_min_degree_vertex(graph, expected):
    ...
    assert (
        undirected.get_vertex_with_min_degree(vertices.values()) == vertices[expected]
    )
